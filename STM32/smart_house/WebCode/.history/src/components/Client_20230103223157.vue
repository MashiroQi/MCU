  ...
  methods: {
    MqttConnect() {
      ...
      client.on("connect", function () {
        ...
        client.subscribe("devtest", function (err) {
          if (!err) {
            console.log("成功订阅!");
          } else {
            console.log("订阅失败!");
    ...
    onLedChange(event) {
      ...
      let sw1 = event.disabled;
      if (sw1) {
        client.publish("test", '{ "LED_SW" : 1 }', function (err) {
          if (!err) {
            console.log("发送成功");
          }
        });
      } else {
        client.publish("test", '{ "LED_SW" : 0 }');
      }
    },
    onBeepChange(event) {
      console.log("beep", event);
      let sw2 = event.disabled;
      if (sw2) {
        client.publish("test", '{"BEEP_SW":1}', function (err) {
          if (!err) {
            console.log("发送成功");
          }
        });
      } else {
        client.publish("test", '{"BEEP_SW":0}');
      }
    },
  },
