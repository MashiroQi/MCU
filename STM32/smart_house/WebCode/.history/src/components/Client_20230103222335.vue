...
export default {
  ...
  methods: {
    ...
    onLedChange(event) {
      ...
      if (sw) {
        client.publish("devtest", '{"LED_SW":1}', function (err) {
          if (!err) {
            console.log("发送成功");
          }
        });
        ...
        client.publish("devtest", '{ "LED_SW":0 }');
        console.log(this.Led);
      }
    },
    onBeepChange(event) {
      ...
      let sw = event.disabled;
      if (sw) {
        client.publish("devtest", '{"BEEP_SW":1}', function (err) {
          if (!err) {
            console.log("发送成功");
          }
        });
        ...
        client.publish("devtest", '{"BEEP_SW":0}');
        console.log(this.Led);
      }
    },
  },
