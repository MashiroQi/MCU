...
{
	//显示当前曲目的索引,及总曲目数
	LCD_ShowxNum(60+0,430,index,3,16,0X80);		//索引
	LCD_ShowChar(60+24,430,'/',16,0);
	LCD_ShowxNum(60+32,430,total,3,16,0X80); 	//总曲目				  	  
}
//显示当前音量
void mp3_vol_show(u8 vol)
{			    
	LCD_ShowString(60+110,430,200,16,16,"VOL:");	  	  
	LCD_ShowxNum(60+142,430,vol,2,16,0X80); 	//显示音量	 
}
u16 f_kbps=0;//歌曲文件位率	
//显示播放时间,比特率 信息 
//lenth:歌曲总长度
...
{	
	...
	else if((time!=playtime)&&(time!=0))//1s时间到,更新显示数据
	{
		...
		//显示播放时间			 
		LCD_ShowxNum(60,400,time/60,2,16,0X80);		//分钟
		LCD_ShowChar(60+16,400,':',16,0);
		LCD_ShowxNum(60+24,400,time%60,2,16,0X80);	//秒钟		
 		LCD_ShowChar(60+40,400,'/',16,0); 	    	 
		//显示总时间
		if(f_kbps)time=(lenth/f_kbps)/125;//得到秒钟数   (文件长度(字节)/(1000/8)/比特率=持续秒钟数    	  
		else time=0;//非法位率	  
 		LCD_ShowxNum(60+48,400,time/60,2,16,0X80);	//分钟
		LCD_ShowChar(60+64,400,':',16,0);
		LCD_ShowxNum(60+72,400,time%60,2,16,0X80);	//秒钟	  		    
//		//显示位率			   
...
void mp3_play(void)
{
	...
	{	
		...
		strcat((char*)pname,(const char*)fn);  			//将文件名接在后面
 		LCD_Fill(60,390,240,190+16,WHITE);				//清除之前的显示
		Show_Str(60,390,240-30,16,fn,16,0);				//显示歌曲名字 
			...
			while(rval==0)
			{
			    ...
			    {  	
					...
					}else   
					{
						...
						{
							if(tp_dev.x[0]<40&&tp_dev.x[0]>0&&tp_dev.y[0]<40&&tp_dev.y[0]>0)
							{
								return 0;
							}
						}
