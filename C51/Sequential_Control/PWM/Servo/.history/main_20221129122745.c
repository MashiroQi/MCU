...
/*
void delay_xus(uint y, uint x) // 延时500us
{
    TMOD = 0x10; // 开定时器1，工作方式为1
    TR1 = 1;     // 启动定时器0；
    while (y--)
    {
        ...
        TH1 = char(65536 - (11.0592 * x) / 12.0) / 256; // 定时500us初值的高八位装入TH0
        TL1 = char(65536 - (11.0592 * x) / 12.0) % 256; // 定时1ms初值的低八位装入TL0
        while (!TF1)
            ;    // 等待，直到TF1为1
        TF1 = 0; // 重置溢出标志位
    }
